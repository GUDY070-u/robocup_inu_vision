...
class Yolo3DNode(Node):
    def __init__(self):
        ...
        # [수정] Open3D 시각화 관련 변수들 제거 (vis, pcd_vis, obj_axis 등 삭제)
        ...
        self.get_logger().info("✅ YOLO 3D Node Started (Visualization Disabled).")
    ...
    def process_callback(self, color_msg, depth_msg):
        if self.intrinsics is None or self.source_pcd is None:
            return
        ...
        if len(boxes) == 0:
            cv2.imshow("Detection (2D)", color_image)
            cv2.waitKey(1)
            return
        ...
        for idx, (x1, y1, x2, y2) in enumerate(boxes.astype(int)):
            ...
            try:
                ...
                cv2.putText(color_image, f"ID:{int(cls_ids[idx])}", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            except:
                pass

        # [수정] 3D 시각화 함수 대신 OpenCV 창만 띄움
        cv2.imshow("Detection (2D)", color_image)
        cv2.waitKey(1)
    ...
    def handle_get_pose(self, request, response):
        ...
        if d["success"]:
            ...
            self.get_logger().info(f"📤 Service Resp: ID={response.detected_id}, X={response.x:.3f}, Y={response.y:.3f}, Z={response.z:.3f}, RZ={response.rz:.2f}")
        else:
