...
class Yolo3DNode(Node):
    ...
    def stabilize_angle_symmetry(self, current, reference, symmetry_step_deg=90.0):
        """알루미늄 프로파일의 대칭성을 고려하여 각도 점프를 억제"""
        step = math.radians(symmetry_step_deg)
        # 이전 값과 현재 값의 차이를 최소화하는 n*step을 찾음
    ...
    def process_callback(self, color_msg, depth_msg):
        ...
        for idx, (x1, y1, x2, y2) in enumerate(boxes.astype(int)):
            ...
            try:
                ...
                # 필터 큐에 저장 (Sin/Cos Median)
                curr_angles = {'roll': r_stable, 'pitch': p_stable, 'yaw': y_stable}
                avgs_deg = {}
                for k in ['roll', 'pitch', 'yaw']:
                    self.angle_queues[k]['sin'].append(math.sin(curr_angles[k]))
                    self.angle_queues[k]['cos'].append(math.cos(curr_angles[k]))
                    m_sin, m_cos = np.median(self.angle_queues[k]['sin']), np.median(self.angle_queues[k]['cos'])
                    res_rad = math.atan2(m_sin, m_cos)
                    avgs_deg[k] = np.degrees(res_rad)
                    self.prev_angles[k] = res_rad # 다음 프레임을 위해 업데이트
    ...
    def update_visualization(self, color, points, final_trans=None):
        cv2.imshow("Final Pose Estimation Overlay", color)
        cv2.waitKey(1)
        if len(points) > 0:
