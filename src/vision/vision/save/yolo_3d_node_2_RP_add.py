...
class Yolo3DNode(Node):
    def __init__(self):
        ...
        self.latest_result = {
            ...
            "roll_deg": 0.0, "pitch_deg": 0.0, "yaw_deg": 0.0, # roll, pitch 추가
            "id": -1
        }
        ...
        self.roll_queue = deque(maxlen=self.window_size)  ##
        self.pitch_queue = deque(maxlen=self.window_size) ##

        self.plot_len = 50
        self.data_x = deque(maxlen=self.plot_len)
        self.data_y = deque(maxlen=self.plot_len)
        self.data_z = deque(maxlen=self.plot_len)
    ...
    def handle_get_pose(self, request, response):
        ...
        if data["success"]:
            ...
            response.rx = float(data["roll_deg"])   # 추가
            response.ry = float(data["pitch_deg"])  # 추가
    ...
    def process_callback(self, color_msg, depth_msg):
        ...
        for idx, (x1, y1, x2, y2) in enumerate(boxes.astype(int)):
            ...
            info_text = f"XYZ:({centroid[0]:.2f},{centroid[1]:.2f},{centroid[2]:.2f}) Conf:{conf_score:.2f}"
            ...
            try:
                ...
                roll_deg, pitch_deg, yaw_deg = np.degrees(roll), np.degrees(pitch), np.degrees(yaw)## 각도 변환 (Rad -> Deg) roll, pitch 추가

                # 이동평균 적용
                self.x_queue.append(centroid[0])
                self.y_queue.append(centroid[1])
                self.z_queue.append(centroid[2])
                self.roll_queue.append(roll_deg)
                self.pitch_queue.append(pitch_deg)##
                self.yaw_queue.append(yaw_deg)##

                avg_x = np.mean(self.x_queue)
                avg_y = np.mean(self.y_queue)
                avg_z = np.mean(self.z_queue)
                avg_roll = np.mean(self.roll_queue)
                avg_pitch = np.mean(self.pitch_queue)
                avg_yaw = np.mean(self.yaw_queue)
                


                rot_text = f"RPY:({int(avg_roll)},{int(avg_pitch)},{int(avg_yaw)})"
                ...
                self.data_roll.append(avg_roll)
                self.data_pitch.append(avg_pitch)
    ...
    def init_plot_thread(self):
        plt.ion()
        # 3행 2열 레이아웃으로 변경 (좌측: XYZ, 우측: RPY)
        self.fig, ((self.ax_x, self.ax_roll), (self.ax_y, self.ax_pitch), (self.ax_z, self.ax_yaw)) = plt.subplots(3, 2, figsize=(12, 8))
        self.fig.suptitle("Moving Average: X,Y,Z,Roll,Pitch,Yaw")
        ...
        self.line_roll, = self.ax_roll.plot([],[],'c-'); self.ax_roll.set_title('Roll'); self.ax_roll.set_ylim(-180,180)
        self.line_pitch, = self.ax_pitch.plot([],[],'y-'); self.ax_pitch.set_title('Pitch'); self.ax_pitch.set_ylim(-180,180)
        self.line_yaw, = self.ax_yaw.plot([],[],'m-'); self.ax_yaw.set_title('Yaw'); self.ax_yaw.set_ylim(-180,180)
    ...
    def update_plot(self):
        ...
        self.line_roll.set_data(range(len(self.data_roll)), list(self.data_roll))
        self.ax_roll.set_xlim(0, self.plot_len)
        self.line_pitch.set_data(range(len(self.data_pitch)), list(self.data_pitch))
        self.ax_pitch.set_xlim(0, self.plot_len)
