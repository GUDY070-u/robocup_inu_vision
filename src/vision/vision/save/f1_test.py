...
class Yolo3DNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("✅ YOLO 3D Node Started. (Filtering Target ID: 11)")
    ...
    def process_callback(self, color_msg, depth_msg):
        ...
        for idx, (x1, y1, x2, y2) in enumerate(boxes.astype(int)):
            ...
            # --- [핵심 수정] ID가 11번인 물체만 처리 ---
            if obj_id != 12:
                # 타겟이 아닌 물체는 화면에 작게 표시만 하고 건너뜀
                cv2.rectangle(color_image, (x1, y1), (x2, y2), (100, 100, 100), 1)
                continue

            # 11번 물체인 경우 시각화 및 ICP 수행
            cv2.rectangle(color_image, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(color_image, f"TARGET ID: {obj_id}", (x1, y1 - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    def handle_get_pose(self, request, response):
        """서비스 요청 시 12번 물체의 최신 정보를 응답"""
        current_time = time.time()
        data_time = self.latest_result.get("timestamp", 0.0)
        
        # 11번 데이터가 있고 0.5초 이내의 데이터인 경우에만 성공 응답
        if self.latest_result["success"] and (current_time - data_time < 0.5):
            ...
            # 응답 후 잔상 방지를 위해 큐 및 상태 초기화
            self.reset_queues()
            self.latest_result["success"] = False 
            
            self.get_logger().info(f"📤 [전송성공] ID {response.detected_id} 좌표 전송 완료.")
        else:
            response.success = False
            self.get_logger().warn("⚠️ [전송실패] 타겟(ID 11)이 현재 보이지 않거나 데이터가 오래되었습니다.")
